...
const process = (getAccessToken, app, axios, moment) => {
  ...
  app.post("/stkpush_acl", async (req, res) => {
    ...
    try {
      ...
      // REMOVE OLD STATUS FOR THIS NUMBER
      delete paymentStatus[mpesa_num];

      // SET NEW PAYMENT TO PENDING
      paymentStatus[mpesa_num] = {
        status: "pending",
        number: mpesa_num,
        id: ID,
        amount: amount_kes
      ...
      console.log("Payment set to pending:", paymentStatus[mpesa_num]);
      ...
      await axios.post(
        url,
        {
          ...
          CallBackURL:
            `https://darajaapi-2.onrender.com/callback_ACL?number=${mpesa_num}&id=${ID}&amount=${amount_kes}`,
      ...
      return res.send(
        "😀 Request sent. Enter MPESA PIN to complete payment"
      );
